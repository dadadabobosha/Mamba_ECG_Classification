...
class MambaBlock(torch.nn.Module):
    def __init__(
        ...
        self.A_log = torch.nn.Parameter(torch.log(A))
        self.D = torch.nn.Parameter(torch.ones(self.expanded_dim, dtype=torch.float32))  # 确保为 float32

        self.out_proj = torch.nn.Linear(self.expanded_dim, self.in_channels, bias=bias).float()  # 确保为 float32

    def forward(self, x):
        L = x.size(1)
        x = x.float()  # 确保输入为 float32
        # Project input x an residual connection z
        x_z = self.in_proj(x)
...
class MambaBEAT(torch.nn.Module):
    def __init__(
        ...
        self.norm = RMSNorm(in_channels).float()  # 确保为 float32
        self.dropout = torch.nn.Dropout(dropout)
        self.linear = torch.nn.Linear(in_channels, out_channels).float()  # 确保为 float32
        self.softmax = torch.nn.Softmax(dim=1)
...
class RMSNorm(torch.nn.Module):
    def __init__(self, size: int, epsilon: float = 1e-5, bias: bool = False):
        ...
        self.epsilon = epsilon
        self.weight = torch.nn.Parameter(torch.ones(size, dtype=torch.float32))  # 确保为 float32
        self.bias = torch.nn.Parameter(torch.zeros(size, dtype=torch.float32)) if bias else None  # 确保为 float32
